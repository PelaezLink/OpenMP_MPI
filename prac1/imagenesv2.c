           ...
           int n, int m)
{
  ...
  int r, g, b; //Variables para mostrar el resultado
...
//El bucle pasos no se puede paralelizar, dependencia entre iteraciones

  for (p = 0; p < pasos; p++) {
    //#pragma omp parallel for private(r, g, b, tot, v, l, k, j) 
    for (i = 0; i < n; i++) {
    #pragma omp parallel for private(r, g, b, tot, v, l, k)
      for (j = 0; j < m; j++) {
        r = 0;
        g = 0;
        b = 0;
