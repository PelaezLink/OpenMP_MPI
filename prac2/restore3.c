...
void swap( Byte a1[],Byte a2[],int rw,int rh,int w ) {
  ...
  if ( a1 != a2 ) {
    ...
    #pragma omp parallel for private (d, x, aux) schedule (runtime)
    for ( y = 0 ; y < rh ; y++ ) {
...
void process( int w,int h,Byte a[], int bw,int bh ) {
  ...
  for ( y = bh ; y < h ; y += bh ) {
    min = INT_MAX; my = y;
    #pragma omp parallel private (min_iter, max_iter, niteraciones, nhilo)
    {
    min_iter = INT_MAX;
    max_iter = INT_MIN;
    niteraciones = 0;
    ...
    for ( y2 = y ; y2 < h ; y2 += bh ) {
      ...
      if (y == bh) {
        if(d < min_iter) {min_iter = d;}
        if(d > max_iter) {max_iter = d;}
        niteraciones++;
      }
      if ( d < min ) { 
        #pragma omp critical
    ...
    if (y == bh) {
        ...
        printf("Hilo %d: %d iteraciones con distancias entre %d y %d.\n", nhilo, niteraciones, min_iter, max_iter);
    }
    }
